...
$(document).ready(// 获取相关指标
	function() {
	    $.ajax({
	        ...
	        success : GetRelatedIndicators
	        // 设置成功后回调函数
	    })
	});

	    // 获取XML成功后回调函数
	    function GetRelatedIndicators(xml1) {
	        ...
	        $("#indicatorName").on("change",function() {
	            countryArray=countryArray.substring(0, 0);
	            countryArray += $("#indicatorName option:selected").attr("value").toLowerCase()+"@";
	            $(xml1).find("indicator[name='"+ $("#indicatorName option:selected").text()+ 
	            		"']>related-indicators").each(
                      function(i) {// 选择点击的节点下面的子节点
                          var country_name = $(this).attr("name");
                          var country_id = $(this).attr("id").toLocaleLowerCase();
	    ...
	    var jscode;
	    function senddata() {
	        var url = "relatedQuery.action?indicatorName=" + showValues()
                    + "&timespanstart=" + getTimeStart() + "&timespanend="
                    + getTimeEnd();
